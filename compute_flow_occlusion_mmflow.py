...
if __name__ == "__main__":
    ...
    for t in range(0, 5*len(frame_list) - 5, 5):
        ...
        ### load input images
        # 我们是png格式的哦
        # img1 = utils.read_img(os.path.join(frame_dir, "%05d.jpg" %(t)))       # 0-1
        ...
        # img2 = utils.read_img(os.path.join(frame_dir, "%06d.png" % (t + 5)))
        img1 = cv2.imread(os.path.join(frame_dir, "%06d.png" % (t)))            # 0-255
        img2 = cv2.imread(os.path.join(frame_dir, "%06d.png" % (t + 5)))
        ...
        with torch.no_grad():
            ...
            # fw_flow = inference_model(model,
            #                          os.path.join(frame_dir, "%06d.png" % (t)),
            #                          os.path.join(frame_dir, "%06d.png" % (t + 5)))
            fw_flow = inference_model(model,
                                     img1,
                                     img2)
